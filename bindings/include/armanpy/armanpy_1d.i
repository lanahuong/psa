...
%fragment( "armanpy_vec_typemaps", "header",
fragment = "armanpy_typemaps"
)
{
...
bool armanpy_numpy_as_vec_with_shared_memory(PyObject *input, VecT **m) {
    ...
    *m = new
    VecT((eT
    *)array_data(array), p, false, false );
    return true;
}
...
%define %
armanpy_vec_byvalue_typemaps( ARMA_MAT_TYPE )
...
{
if( !
armanpy_basic_typecheck<ARMA_MAT_TYPE>( $input, true
))
SWIG_fail;
array = obj_to_array_no_conversion($input, ArmaTypeInfo<ARMA_MAT_TYPE>::type);
if( !array )
SWIG_fail;
$1 = ARMA_MAT_TYPE((ARMA_MAT_TYPE::elem_type * )
array_data(array), arma::uword(array_dimensions(array)[0]), false
);
}
...
%define %
armanpy_vec_const_ref_typemaps( ARMA_MAT_TYPE )
...
SWIG_fail;
$1 = new
ARMA_MAT_TYPE(( ARMA_MAT_TYPE::elem_type
*)
array_data(array),
...
%define %
armanpy_vec_ref_typemaps( ARMA_MAT_TYPE )
...
{
if( !
armanpy_basic_typecheck<ARMA_MAT_TYPE>( $input, true, true
)) {
PyErr_SetString( PyExc_RuntimeError,
"Argument not a valid armadillo matrix" );
SWIG_fail;
}
if( !
armanpy_numpy_as_vec_with_shared_memory <ARMA_MAT_TYPE>( $input,
&($1))) {
PyErr_SetString( PyExc_RuntimeError,
"Numpy array can not be wrapped as armadillo vector" );
SWIG_fail;
}
}
...
%define %
armanpy_vec_return_by_value_typemaps( ARMA_MAT_TYPE )
%
...
typemap ( out )
    ...
    if (!array) {
        PyErr_SetString(PyExc_RuntimeError, "Return by value failed (armanpy_vec_copy_to_numpy).");
        SWIG_fail;
    }
...
%define %
armanpy_vec_return_by_reference_typemaps( ARMA_MAT_TYPE )
%
...
if ( !array ) {
PyErr_SetString( PyExc_RuntimeError,
"Return by reference failed (armanpy_vec_copy_to_numpy)." );
SWIG_fail;
}
$result = SWIG_Python_AppendOutput($result, array);
}
