...
%fragment( "armanpy_cube_typemaps", "header",
...
bool armanpy_numpy_as_cube_with_shared_memory(PyObject *input, MatT **m) {
    ...
    *m = new
    MatT((eT
    *)array_data(array), r, c, s, false, false );
    return true;
}
...
%define %
armanpy_cube_byvalue_typemaps( ARMA_MAT_TYPE )
...
%define %
armanpy_cube_const_ref_typemaps( ARMA_MAT_TYPE )
...
%define %
armanpy_cube_ref_typemaps( ARMA_MAT_TYPE )
...
"Argument not a valid armadillo cube" );
SWIG_fail;
}
if( !
armanpy_numpy_as_cube_with_shared_memory <ARMA_MAT_TYPE>( $input,
&($1))) {
PyErr_SetString( PyExc_RuntimeError,
"Numpy array can not be wrapped as armadillo cube" );
SWIG_fail;
}
}
...
%define %
armanpy_cube_return_by_value_typemaps( ARMA_MAT_TYPE )
%
...
typemap ( out )
    ...
    if (!array) {
        PyErr_SetString(PyExc_RuntimeError, "Return by value failed (armanpy_cube_copy_to_numpy).");
        SWIG_fail;
    }
...
%define %
armanpy_cube_return_by_reference_typemaps( ARMA_MAT_TYPE )
%
...
if ( !array ) {
PyErr_SetString( PyExc_RuntimeError,
"Return by reference failed (armanpy_cube_copy_to_numpy)." );
SWIG_fail;
}
$result = SWIG_Python_AppendOutput($result, array);
}
