...
%fragment( "armanpy_mat_typemaps", "header",
...
bool armanpy_numpy_as_mat_with_shared_memory(PyObject *input, MatT **m) {
    ...
    *m = new
    MatT((eT
    *)array_data(array), r, c, false, false );
    return true;
}
...
%define %
armanpy_mat_byvalue_typemaps( ARMA_MAT_TYPE )
...
array_data(array), array_dimensions(array)[0], array_dimensions(array)[1], false
);
}
...
%define %
armanpy_mat_const_ref_typemaps( ARMA_MAT_TYPE )
...
%define %
armanpy_mat_ref_typemaps( ARMA_MAT_TYPE )
...
if( !
armanpy_numpy_as_mat_with_shared_memory <ARMA_MAT_TYPE>( $input,
&($1))) {
PyErr_SetString( PyExc_RuntimeError,
"Numpy array can not be wrapped as armadillo matrix" );
SWIG_fail;
}
}
...
%define %
armanpy_mat_return_by_value_typemaps( ARMA_MAT_TYPE )
%
...
typemap ( out )
    ...
    if (!array) {
        PyErr_SetString(PyExc_RuntimeError, "Return by value failed (armanpy_mat_copy_to_numpy).");
        SWIG_fail;
    }
...
%define %
armanpy_mat_return_by_reference_typemaps( ARMA_MAT_TYPE )
%
...
if ( !array )  {
PyErr_SetString( PyExc_RuntimeError,
"Return by reference failed (armanpy_mat_copy_to_numpy)." );
SWIG_fail;
}
$result = SWIG_Python_AppendOutput($result, array);
}
