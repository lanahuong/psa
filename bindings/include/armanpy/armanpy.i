...
define array_set_data(arr, mem)(((PyArrayObject *)
...
define array_clear_flags(arr, flg)(((PyArrayObject *)

arr)->flags) = ((((PyArrayObject*)arr)->flags) & ~( flg ))
%#

define array_free_data(arr)

PyArray_free( arr )
%#else
%#

define array_set_base_object(arr, obj)

PyArray_SetBaseObject(arr, (PyObject
*)obj )
%#

define array_set_data(arr, mem)(((PyArrayObject_fields *)
...
define array_clear_flags(arr, flg)

PyArray_CLEARFLAGS( arr, flg
)
%#
...
%fragment( "armanpy_typemaps", "header",
fragment = "NumPy_Fragments", fragment = "ArmaNumPy_Backward_Compatibility"
)
{
...
bool armanpy_basic_typecheck(PyObject *input, bool raise, bool check_own_data = false) {
    ...
    if (armanpy_allow_conversion_flag) {
        PyErr_SetString(PyExc_TypeError,
                        "Conversion not supported anymore. Please wrap your data using numpy.array( ... ) and call armanpy_conversion( false ).");
        return false;
    } else {
        ...
        if (!require_dimensions(array, ArmaTypeInfo<ArmaT>::numdim)) {
            if (raise)
                PyErr_Format(PyExc_TypeError, "Array with %i dimension required. A %i-dimensional array was given.", ArmaTypeInfo<ArmaT>::numdim,
                             array_numdims(array));
            return false;
        }
        ...
        if (sizeof(npy_intp) > sizeof(arma::uword)) {
            ...
            for (npy_intp i = 0; i < ndim; i++) {
                if (dims[i] > max_arma_uword) {
                    if (raise)
                        PyErr_Format(PyExc_TypeError, "Dimension %li of array to large (%li). Only %li elements per dimension supported.", i, dims[i],
                                     max_arma_uword);
                    return false;
                };
            }
