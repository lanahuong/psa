...
void SchemeFTCS::step() {
    ...
    cx mat_pot_factor = m_e_scale_ * step_scale_ * step_scale_ / (h_bar_scale_ * h_bar_scale_);
    cx prefactor = m_e_scale_ * step_scale_ / h_bar_scale_;
    arma::cx_mat centerpart = (mat_pot_ * mat_pot_factor % phit_) + ((prefactor * i * h_bar_ / dt_) + (h_bar_ * h_bar_ / (m_e_ * dx_ * dx_)) + (h_bar_ * h_bar_ / (m_e_ * dy_ * dy_))) * phit_;
    arma::cx_mat tmp = xpart + ypart + centerpart.submat(1, 1, size(phitdt_));
    phitdt_ = tmp * (dt_ / (i * h_bar_ * prefactor));
