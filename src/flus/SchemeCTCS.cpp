...
void SchemeCTCS::step() {
    ...
    const arma::cx_mat f_t_pm_dx = phit_(2, 1, size(phitdt_)) + phit_(0, 1, size(phitdt_));
    const arma::cx_mat f_t_pm_dy = phit_(1, 2, size(phitdt_)) + phit_(1, 0, size(phitdt_));

    const auto pot_sub = mat_pot_(1, 1, size(phitdt_));
    const cx h2m = h_bar_ * h_bar_ / (m_e_ * step_scale_ * step_scale_);
    const arma::cx_mat fact = 1. / (n2 * cx(0, 2 * h_bar_ / (dt_ * step_scale_)) - h2m * (1. / (dx_ * dx_) + 1. / (dy_ * dy_)) - n1 * pot_sub);
    const arma::cx_mat fact2 = n1 * pot_sub + n2 * cx(0, 2 * h_bar_ / (dt_ * step_scale_)) + h2m * (1. / (dx_ * dx_) + 1. / (dy_ * dy_));
    ...
    do {
        ...
        arma::cx_mat res = -(h2m / (2. * dx_ * dx_)) * (f_t_pm_dx + gn_pm_dx) - (h2m / (2. * dy_ * dy_)) * (f_t_pm_dy + gn_pm_dy) + fact2 % phit_(1, 1, size(phitdt_));
        g_np1(1, 1, size(phitdt_)) = res % fact;
